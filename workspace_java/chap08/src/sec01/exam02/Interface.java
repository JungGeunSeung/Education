...
public interface Interface {
	...
	// 실체 메소드를 가지고 있어야 한다. 이러한 객체를 구현(implements)객체라고 한고 구현객체를 생성하는 클래스를 구현클래스라고 한다.
	// 보통의 클래스와 동일하지만, 인터페이스 타입으로 사용할수 있을을 알려주기 위해 클래스 선언부에 implements 키워드를 추가하고 인터페이스 이름을 적는다
	...
	// 인터페이스를 전부 구현한 상태에서 새롭게 메소드를 추가하는 경우, 구현되는 클래스도 전부 override 해줘야 하기 때문에, 불편함이 있다.
	// 그래서 인터페이스 클래스 안에 있는 메소드 앞에 default를 적어주면, 인터페이스에서는 사용이 불가하던 실행블럭을 적을 수 있기 때문에,
	// 구현되는 클래스는 해당 메소드를 선택하여 쓸수 있다.
	// RemoteControl.java 참조
	
	// static 메소드는 override가 되지 않는다.
}
