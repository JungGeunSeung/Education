...
--    maxvalue 90 -- 생성할 최댓값
--    minvalue 0 -- 생성할 최솟값
--    nocycle -- 최댓값에 도달했을때 cycle이면 다시 시작 값에서 시작, nocycle이면 중단
...
-- 생성된 시퀀스를 nextval으로 한번도 실행하지 않은 경우
-- currval은 사용이 불가하다
select seq_test.currval from dual;

------------------------------------------------------------------------------
-- 6월 28일 금요일 수업
-- 제약조건 (테이블을 생성할때 주로 지정하지만 수정,삭제 할때 도 사용되기 때문에 DDL에 활용된다
-- not null : 지정한 열에 null을 허용하지 않습니다. null을 제외한 데이터의 중복은 허용됩니다
-- unique : 지정한 열이 유일한 값을 가져야 한다. 즉 중복 될수 없다. null은 중복에서 제외
-- primary key : 지졍한 열이 유일한 값이면서, null을 허용하지 않는다. (테이블에 하나만 지정이 가능하다)
-- foreign key : 다른 테이블의 열을 참조하여 존재하는 값만 입력할 수 있다
-- check : 설정한 조건식을 만족하는 데이터만 입력이 가능

-- 데이터 무결성 (data integrity) : 데이터베이스에 저장되는 데이터의 정확성과 일관성을 보장한다

-- 영역 무결성 (domain intergrity) : 열에 저장되는 값의 적정 여부를 확인. 자료형, 데이터, null여부같은 정해놓은 범위를 만족하는 데이터
-- 개체 무결성 (entity intergrity) : 테이블 데이터를 유일하게 식별할 수 있는 기본키는 반드시 값을 가지고 있어야 하며 null이 될 수 없고 중복될 수도 없을을 규정
-- 참조 무결성 (referential intergrity) : 참조 테이블의 외래키 값은 참조 테이블의 기본키로서 존재하며 null이 가능

-- primary key = pk 주요키 중요키 기본키라고도 불린다
-- not null + unique 를 합친 조건
-- 생성과 동시에 index도 생성
-- create table에 하나만 지정가능
...
select * from user_constraints; -- user_constraints : 사용자가 추가한 모든 제약조건을 확인하는 테이블
    
select * from user_indexes; -- table_pk 에 primary key를 지정했기 때문에 index도 자동으로 생성 되었다

create table table_pk2 (
    login_id varchar(20) constraint pk_login_id primary key,
    login_pwd varchar(20) constraint nn_login_pwd not null,
    tel varchar(20)
);
...
insert into table_pk2
values ('id','pwd', null); -- 똑같은 값이 한번더 들어가면 error: unique constraint (SCOTT2_5.PK_LOGIN_ID) violated 라고 뜬다

-- create table 에 컬럼명 컬럼타입을 지정하지만, primary key (컬럼명) 으로도 키지정이 가능하다
create table table_pk3 (
    login_id varchar(20),
    login_pwd varchar(20),
    tel varchar(20),
    
    primary key (login_id, login_pwd) -- 원래는 하나밖에 지정하지 못하지만, 두개이상 작성할 경우 두개의 관계를 unique 조건을
                                      -- 걸어서 두개 동시에 중복만 되지 않는다면, promary 할 수 있다.
                                      -- 제약조건 사전에는 하나만 등록된다
);
...
-- 서로 컬럼명은 달라도 된다 (보통 같게 한다)
-- 대상이 되는 컬럼은 pk여야 한다
create table emp_fk (
    empno number primary key,
    ename varchar2(10),
    deptno number references dept_fk (deptno1) -- 컬럼명이 같다면 생략 가능
);
...
-- 수정, 삭제 불가
update emp_fk set deptno = 101; -- dept_fk 에서 없는 값이기 때문에 사용불가
update dept_fk set deptno1 = 101; -- emp_fk 에서 할당되어 있는 값이 있기 때문에, 값을 변경 할수 없다
-- 참조하는 값이 없어야 수정 또는 삭제가 가능하다
-- 참조값도 마찬가지로 참조되고 있는 값이 없어야 수정이 가능하다
-- 만약 pk의 값을 수정하고 싶을때는, fk의 값들을 삭제하고 pk의 값을 변경해야 한다
...
create table emp_fk (
    ...
    foreign key (deptno) references dept_fk (deptno1) -- 컬럼명이 같다면 생략 가능
);

-- default : 특정 열에 저장할 값이 지정되지 않았을 경우에 기본값을 지정 할수 있다
...
---------------------------------------------------------------------------
-- 사용자 관리
-- 스키마(schema) : 오라클 데이터베이스에 접속한 사용자와 연결된 객체 index,sequence,table 등을 뜻한다
-- 즉 사용자의 계정으로 만든 모든 객체는 사용자계정의 스키마가 된다

-- 사용자 생성
--create user 사용자이름
--identified by 패스워드
--default tablespace 테이블 스페이스 이름
--temporary tablespace 테이블(그룹) 스페이스 이름
--quota 테이블 스페이스크기 on 테이블 스페이스 이름
--profile 프로파일 이름
--password expire
--account [lock/unlock]; (둘중 하나 선택)

-- 시스템권한
-- 데이터베이스의 사용자 생성과 정보 수정및 삭제, 데이터베이스 접근, 객체생성 및 관리등의 권한을 설정한다
